...
with st.form('Questions'): # 질문을 입력하는 칸을 페이지로 설정함
    text = st.text_area('Escribe tu pregunta: ',
                        "¿Qué tipo de modelos de texto proporciona OpenAI?")
    # 질문을 입력하는 칸과 예시 질문 입력
    submitted = st.form_submit_button('ENVIAR') # 질문 엽력 후 버튼 활성화
    generate_response(text)
    # 버튼이 눌리면 입력된 질문이 위에서 만든 generate_response 에 input_text로 들어감